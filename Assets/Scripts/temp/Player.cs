...
public class Player : MonoBehaviour
{
  ...
  [Header("Hunger Settings")]
	[SerializeField] private float hungerSpeedMultiplier = 2f;
	[SerializeField] private float maxHungerResource = 100f;
	[SerializeField] private float hungerDrainRate = 25f;    // Units per second
	[SerializeField] private float hungerRecoveryRate = 15f; // Units per second
	[SerializeField] private float hungerRecoveryDelay = 1f; // Seconds to wait before recovery starts
	[SerializeField] private UIPanel hungerBarPanelPrefab;
	[SerializeField] private Transform hungerBarTransformReference;
  ...
  private void SetupHungerBar()
	{
		if (UIManager.Instance != null)
		{
			hungerBarPanel = UIManager.Instance.CreateWorldPositionedPanel(
				hungerBarTransformReference,
				hungerBarPanelPrefab,
				Vector3.zero
			);
